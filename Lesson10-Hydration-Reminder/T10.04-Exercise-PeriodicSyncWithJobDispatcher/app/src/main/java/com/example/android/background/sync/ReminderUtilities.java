...
public class ReminderUtilities {
    ...
    synchronized public static void scheduleChargingReminder(Context context){
        if (sInitialized) return; //if job has been set up then return
        Driver driver = new GooglePlayDriver(context);
        FirebaseJobDispatcher dispatcher = new FirebaseJobDispatcher(driver);
        // let schedual job with constrain schedual
        Job constraintReminderJob = dispatcher.newJobBuilder()
                ...
                .setRecurring(true)
                .setTrigger(REMINDER_INTERVAL_SECONDS,REMINDER_INTERVAL_SECONDS+SYNC_FLEXTIME_SECONDS)
